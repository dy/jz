...
**ProgramIndex:** `src/compile/program-index.js` owns stable numeric identities, same-module member-call targets, direct call edges, export/module/address-taken roots, SCC spans, and reachability for the prepared/imported function snapshot. It is built once after source-mutating plan passes settle, finalized after computed call observations, and then published at `ctx.plans.programIndex`. Consumers resolve a numeric ID first and use `functionById` only when the existing emitter still needs the live function record. Later specialization variants still use the existing registry until final function identity migrates. Do not restore a second member-target table, name-keyed target cache, or call-graph writer.
