...
const PROGRAMS = [
  ...
  { name: 'bitwise mix', src: scalarCase('differential-bitwise-mix').source, args: () => [num(), num()] },
  // imul with a literal ≥ 2³¹ (Knuth's multiplicative hash constant) — exercises ToInt32-wrapping of the operand.
  { name: 'imul big literal', src: scalarCase('differential-imul-literals').source, args: () => [num()] },
  ...
  { name: 'fib-ish', src: scalarCase('differential-fib-i32').source, args: () => [num()] },
  ...
  { name: 'fnv hash', src: scalarCase('differential-fnv-i32').source, args: () => [num(), num(), num()] },
