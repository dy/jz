...
import { EQ_ZERO_KERNEL, EQ_ZERO_REUSE_B, GATHER_MAP_KERNEL, BOUNDED_GATHER_KERNEL } from './_optimizer-kernels.js'
...
const SAMPLES = [
  ...
  ['interrupted-opposing-recurrence', `function f(n){const a=new Float64Array(16);for(let i=0;i<16;i++)a[i]=i+1;let p=1,s=0;for(let i=0;i<(n&31);i++){p++;if(i&1)continue;const j=p|0;s+=a[j]+a[j+1];p--}return[p,s]}export function main(){return[f(1),f(31),f(0),f(31)]}`, [[1,7],[16,NaN],[1,0],[16,NaN]]],
  ['string-collection-probes', `function f(n){const m=new Map(),s=new Set(),key=n?'héllo🙂':'';m.set(key,7);s.add(key);let calls=0;function get(){calls++;return key}const value=m.get(get(),m.set(key,9));return [value,m.has(get()),s.has(get()),calls]}export function main(){return [f(0),f(1),f(0)]}`, [[9,true,true,3],[9,true,true,3],[9,true,true,3]]],
  ['bulk-method-spreads', `export function main(){let a=[7,8],old=a;const b=[2,3];const p=a.push(...(a=b));const u=old.unshift(1,...old);return [a,old,p,u]}`, [[2,3],[1,7,8,2,3,7,8,2,3],4,9]],
  ['fixed-method-spreads', `function f(k){const a=new Int32Array([7,8,9]),args=k?[1,2]:[];function add(x,y){return x===undefined?100:x+y}const cb=k?[add,undefined]:[add];return [Array.from(a.slice(...args)),a.reduce(...cb),new Map().set(...[7,true]).get(7)]}export function main(){return [f(0),f(1),f(0)]}`, [[[7,8,9],24,true],[[8],117,true],[[7,8,9],24,true]]],
  ['bigint-origin-truthiness', `export function main(){const a=new BigInt64Array(2);let calls=0;a.find(x=>{calls++;return x});const key='0';return [calls,a.some(x=>x),typeof a[key],a[key]]}`, [2,false,'bigint',0n]],
  ['empty-callback-spread', `function f(k){const args=k?[undefined]:[];try{new Int32Array(0).map(...args);return 0}catch(e){return e.name}}export function main(){return [f(0),f(1),f(0)]}`, ['TypeError','TypeError','TypeError']],
  ['nullable-bigint-method-receiver', `const a=new BigInt64Array([7n,9221120245631025152n]);function f(i){try{return a[i].toString(10)}catch(e){return e.name}}export function main(){return [f(0),f(1),f(2),f(0)]}`, ['7', '9221120245631025152', 'TypeError', '7']],
  ['nullable-method-receiver', `function f(k){let calls=0;const xs=[new Int32Array([7])];const a=xs[k];function pos(){calls++;return 0}try{return [a.at(pos()),calls]}catch(e){return [e.name,calls]}}export function main(){return [f(0),f(1),f(0)]}`, [[7, 1], ['TypeError', 0], [7, 1]]],
  ['method-positions', `function part(a,e){return Array.from(a.subarray('1',e))}export function main(){const a=new Int32Array([7,8,9]);let error;try{a.at(1n)}catch(e){error=e.name}return [part(a,undefined),part(new Uint8Array([7,8]),2),a.at(4294967295>>>0),error]}`, [[8, 9], [8], undefined, 'TypeError']],
  ['typed-computed-accessors', `function field(key){const a=new Uint16Array(4).subarray(1);return a[key]}export function main(){return field('length')+field('byteLength')+field('byteOffset')}`, 11],
  ['nullable-typed-receiver', `function read(i,key){const xs=[new Float64Array([7])];try{const a=xs[i];return a[key]}catch(e){return e.name}}export function main(){return [read(0,'0'),read(1,'0'),read(0,'length')]}`, [7, 'TypeError', 1]],
  ['nullable-direct-receiver', `export function field(i){const xs=[{x:7}];try{const a=xs[i];return a.x}catch(e){return e.name}}export function index(i){const xs=[new Float64Array([9])];try{const a=xs[i];return a[0]}catch(e){return e.name}}export function main(){return [field(0),field(1),index(0),index(1)]}`, [7, 'TypeError', 9, 'TypeError']],
  ['nullable-loop-zero-work', `let p;function sum(n){let s=0;for(let i=0;i<n;i++)s+=p[i];return s}export function main(){const zero=sum(0);let error;try{sum(1)}catch(e){error=e.name}p=new Float64Array([2,3,4]);return [zero,error,sum(3)]}`, [0, 'TypeError', 9]],
  ['nullable-bigint-arithmetic-local', `const a=new BigInt64Array([2n,3n]);export function f(i){i|=0;const d=a[i]-a[i];return [a[i],d]}export function main(){return [f(0),f(2)]}`, [[2n, 0n], [undefined, NaN]]],
  ['explicit-bigint-number', `const a=new BigInt64Array([7n]);function f(i){try{return 'abc'.charAt(a[i])}catch(e){return e.name}}export function main(){return [Number(a[0]),Number(a[1]),f(0),f(1),Number({valueOf(){return 8n}})]}`, [7, NaN, 'TypeError', 'a', 8]],
  ...
  ['string-operands', `export let main = () => { const s='abcdefgh'+1; return s.concat(true)+'|'+s+'|'+ 'a7b'.split(7n)[1]+'|'+ 'xtrue'.search(true)+'|'+ 'x'.padStart(5,true) }`, 'abcdefgh1true|abcdefgh1|b|1|truex'],
...
test('self-compile: fractional bounds and shared gather addresses survive compiler reuse', () => {
  ...
  for (const [length, advance] of [[16, 0], [16, 0], [16, 20], [0, 0], [8, 0], [16, 0]]) {
    ...
    const actual = instantiate(s.memory.read(s.exports.default(input, 0, options)).slice()).exports.probe
    const expected = jz(src, { optimize: 'speed' }).exports.probe
    for (const n of [0, 1, 2, 3, 30, 31]) for (const pick of [-1, 0, Math.max(0, n - 1), n])
      is(actual(n, 0, 0, pick), expected(n, 0, 0, pick), `bounded length ${length}, count ${n}, pick ${pick}`)
  }
})
