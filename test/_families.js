...
export const FAMILIES = [
  ...
  { family: 'BigInt across an internal call boundary, then a typed store', cases: [
    ...
    { name: 'a boxed BigInt into a parameter of every kind (watr optimize.js slebSize)', src: `const size = (v) => { let x = typeof v === 'bigint' ? v : typeof v === 'string' ? BigInt(v) : BigInt(Math.trunc(Number(v) || 0)); let n = 1; while (true) { const b = x & 0x7fn; x >>= 7n; if ((x === 0n && (b & 0x40n) === 0n) || (x === -1n && (b & 0x40n) !== 0n)) return n; n++ } }\nconst node = ['i64.const', 300n]\nexport let f = (k) => k === 1 ? size(node[1]) : k === 2 ? size('300') : k === 3 ? size(true) : size(k)`, calls: [['f', [1], 2], ['f', [2], 2], ['f', [3], 1], ['f', [0], 1]],
      red: { native: 'every level: the caller boxes the element, the callee reads it raw; the parameter of every kind never materializes (the BOOL veto, representation-plan/body-data.js hasClosedBool), and without the veto the kernel fails to compile itself' } },
