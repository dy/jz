...
test('Math.pow / ** — a constant non-integer exponent takes the pow kernel, bit-identical to a runtime exponent', () => {
  // A constant NON-integer exponent (one the k/5 fifthroot fold does not cover) lowers to the
  // same $math.pow call a runtime exponent takes: one implementation, within an ulp of the
  // host's fdlibm pow, every JS special case intact. exp(c·log x) used to be a second
  // algorithm here, ~1e-9 relative off $math.pow and NaN at -∞.
  const m = run(`
  ...
  `)
  for (const x of [0, -0, 0.5, 1, 2, 1.1, 3.14159, 47.032, 1e150, 1e-300, Infinity, Number.MIN_VALUE, -Infinity, -3, NaN]) {
    is(m.g45(x), m.ref(x, 0.45), `0.45: x=${x}`)
    is(m.gsrgb(x), m.ref(x, 1 / 2.4), `1/2.4: x=${x}`)
    is(m.gneg(x), m.ref(x, -1.5), `-1.5: x=${x}`)
  }
  ok(Number.isNaN(m.g45(NaN)), 'NaN → NaN')
  ok(Number.isNaN(m.g45(-3)), '(-finite)**c = NaN')
  is(m.g45(-Infinity), Infinity, '(-∞)**0.45 = +∞, as Math.pow')
  is(m.gneg(-Infinity), 0, '(-∞)**-1.5 = 0, as Math.pow')
...
// maybeUndefined gate (.work/archive/todo.md §deletion-sweep Slice 2): a dict/map
