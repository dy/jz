...
const WASM_TODO = {
  noise:    'needs SLP: the 4 independent Perlin corner gradients are isomorphic straight-line trees that should pack into f64x2 lanes (rustc/clang do). jz codegen is otherwise clean scalar — no f64/dispatch leak. Prereq landed (84aee0e: the per-gradient NaN-canon is now stripped through the ?: sign-select, so the grad trees are SIMD-isotropic); the f64x2 pack of the quad-grad is the remaining work.',
