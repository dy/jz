...
const WASM_TODO = {
  ...
  fft:      'FMA-parity floor, ~1.05× behind rust-wasm: jz keeps the butterfly NON-fused (mul then add) so its checksum stays bit-identical to JS/V8 (the differential contract), while rust-wasm auto-FMAs the mul-add for the ~5%. Closing it needs either butterfly f64x2 vectorization or accepting an FMA parity class for fft — the latter breaks valid-jz=valid-JS, so NOT a free win. DESIGN (shape verified): 2-wide over consecutive j — re[a],re[a+1] / im[a],im[a+1] / re[b],re[b+1] / im[b],im[b+1] are all ADJACENT pairs (b = a+half, a second loop-invariant base), twiddles wre[k],wre[k+step] are runtime-strided so they lift as two scalar loads + lane combine, the +/−/× lanes with NO reassociation and NO fusion (checksum-identical per construction), and half is even for every len≥4 so there is no scalar tail. Blocked on the scaffold: the loop has TWO IVs (j++, k+=step) and matchBlockLoop accepts one — the lift never attempts it. Needs a dual-IV affine extension (k = j·step derivable) + paired read-write lanes; SIMD-adjacent, so it lands only with the differential fuzz gate.',
  raytrace: 'residual SCALAR-codegen-quality gap (1.24× rust, was 1.83×). Alias-analysis LICM (477624a) already hoisted the read-only sphere loads out of the pixel loop — raytrace now BEATS c-wasm. The remaining gap is rust`s tight 0-SIMD scalar (86 f64.mul/46 add/10 sqrt; clang does NOT autovectorize the sphere loop). Closing it needs the iteration-invariant `c = ox²+oy²+oz²-r²` hoisted out of the pixel loop (measured ceiling: ties rust ~1.0×, never beats). That is blocked by unroll SCRATCH-SHARING: the sphere loop is unrolled by unrollTypedArrayLoops (plan/literals.js), so `const ox` collapses to one $ox set 8× — a multi-def the LICM cannot hoist. The fix would be scalar-renaming the unrolled scratch (each iteration its own locals), BUT that same unroll FEEDS the lane vectorizer, which recombines iterations by matching shared local names across them — renaming defeats it (BUILT + measured: 4 SIMD tests broke with "local not in scope", the vectorizer could no longer fuse the lanes). So the scratch-sharing is load-bearing for vectorization, not an oversight; un-sharing it for LICM and keeping it for SIMD are in direct conflict, and the win only ties rust anyway. Earlier-disproven: SLP (rust is 0-SIMD), unroll-bloat (staying-a-loop measured SLOWER 3.19 vs 3.03 ms). The floor is LLVM scalar instruction-selection/regalloc, which V8`s wasm tier caps anyway. RE-VERIFIED (lowering lab): hand register-hoisting all per-sphere invariants into unrolled locals = 1495µs vs rust-wasm 1487 — a TIE, never a beat, confirming the ceiling. And stripping EVERY NaN-canon select from render = 0µs gain (1494 vs 1496) — V8 folds `select(nan,x,x≠x)` for free, so the canon is not the residual. The residual really is pure scalar scheduling. Closing to a tie needs the risky scratch-rename for no speed win → not worth it.',
  ...
  strbuild: 'per-record int→string + concat (the serializer inner loop) — rust-wasm leads ~1.3× (zig-wasm formats into a stack buffer, ~4.5×, not a gate rival). jz allocates every concat temporary through __alloc + __str_concat and renders ints via the generic __itoa; the levers are itoa-into-arena and a small-concat scratch path that fuses `a + b + c + d` chains into one sized allocation.',
