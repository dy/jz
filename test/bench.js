...
const WASM_TODO = {
  noise:    'scalar codegen is clean (no f64/dispatch leak; per-gradient NaN-canon stripped, 84aee0e). The "pack the 4 gradients into f64x2" SLP hypothesis is DISPROVEN for the jz→V8 pipeline: a hand-vectorized perlin using jz`s own f64x2 intrinsics (bit-exact) measured 1.87 ms vs 1.13 ms scalar — 65% SLOWER. The 4 gradients need DIFFERENT inputs per lane (xf vs xf-1, yf vs yf-1, per-corner sign), so f64x2.lanes lane-construction + extracts cost more than V8`s already-cmov`d scalar branches; the perm[] gather stays scalar either way. Do not build the quad-grad SLP — it regresses. Any future win must come from a tighter SCALAR shape, not SIMD.',
  ...
  raytrace: 'fundamental SCALAR-codegen-quality gap, NOT vectorization. DISPROVEN by disassembling the rival: rust-wasm`s winning raytrace is 0-SIMD tight scalar (86 f64.mul / 46 add / 10 sqrt, no f64x2/v128) — clang does NOT autovectorize the sphere loop, it just emits tighter scalar than jz→V8. (Also disproven earlier: the unroll-bloat hypothesis — forcing the trip-8 sphere loop to stay a loop measured SLOWER, 3.19 vs 3.03 ms.) No jz vectorizer pass closes this; only LLVM-grade scalar instruction-selection/regalloc would, which V8`s wasm tier caps anyway.',
