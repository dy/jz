...
const WASM_TODO = {
  ...
  strbuild: 'per-record int→string + concat (the serializer inner loop) — rust-wasm leads ~1.3× (zig-wasm formats into a stack buffer, ~4.5×, not a gate rival). DE-STALED 2026-07-13: chain fusion ALREADY EXISTS (emit.js fuseConcatChain, ≥3 leaves → one strcat allocation — a prepare-level duplicate was built and reverted against it). The REAL residual, profiled quiet (names:true): __str_copy 22.7% + __str_byteLen 16.0% dominate — SIX tiny-part copies and length calls per row, pure call overhead on 1-6 byte pieces; __itoa is only 13.4%. The levers are strcat-emitter inlining: SSO parts unpack to direct byte stores (no __str_copy call), and SSO/plain-heap lengths extract inline (bit field / header load, no __str_byteLen call).',
