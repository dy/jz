...
const WASM_TODO = {
  sdf:      "jz ~6.9 ms vs c-wasm ~5.3 ms. The checked reads on the hull cursor k (v[k], z[k], f[v[k]]: 13 f64 sites) are the gap: dropping them by hand (WAT surgery, conditions kept) measured 5.75 ms, 72% of the way to C. k is bounded below only by the sentinel z[0] = −INF, a semantic invariant no static hull proves, and above by q < n; f[v[k]] needs v's elements below n, a relational hull with a runtime n. A per-iteration `k ≥ 1` version of the inner while would keep one compare where three reads check now; the relational hull is the research tail.",
  ...
  glyfparse: "jz ~3.2 ms vs c-wasm ~2.6 ms. Not the bounds checks: dropping every stream and flagBuf check by hand (WAT surgery, conditions kept) measured no change. The flag loop carries each stream byte as the checked read's Number|undefined f64, so `flagBuf[p++] = f` is a `__to_int32` call and `f & 8` a guarded ToInt32; folding that call to an inline i64 truncation measured 17% SLOWER (6-round A/B). The lever is representation: a checked Uint8Array read whose every use is ToInt32-blind (a typed store, `&`, a compare) carried as an i32 with a zero miss.",
  noise:    "jz ~1.66 ms vs rust-wasm ~1.45 ms (1.13×; it led at 84af634, 0.90×). The per-octave op census matches rust within a few ops; the ToInt32 guards and the perm bounds checks measured no cost (dropped by hand, no change), and unrolling the four octaves measured 3.6% slower than the loop, so a small-constant unroll now stops at 1000 body nodes. SLP stays disproven (the four corners need different per-lane inputs; a hand-vectorized perlin ran 65% slower). The residual is V8-codegen-level.",
  ...
  sort:     "heapsort sift-down, 1.0–1.09× zig-wasm paired (0.76× rust, 0.93× c). Two lowering defects, both fixed: watr's ifset folded the child pick `if (child+1 < n && a[child] < a[child+1]) child++` into a select over a branchy condition, which V8 runs 35% slower than the branch (ifset now keeps branchy conditions); and the sift reloaded a[i]/a[child] after the compare (load CSE now survives an exit-only if). The residual is the dependent-load chain.",
