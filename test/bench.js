...
const WASM_TODO = {
  ...
  glyfparse: "jz 1.06× c-wasm in 16 alternating rounds (1.16× before twin locals). Not the bounds checks: dropping every stream and flagBuf check by hand measured no change. The versioned coordinate loops now accumulate in i32 (compile/twin-locals.js: the checked twin writes locals of its own), matching a hand-hinted scratch variant op for op. The rest is the flag loop: `rep = stream[r++]` stays f64 because its checked read may miss, though its only uses are `rep > 0` and a decrement that test guards (undefined and 0 both fail it). Hinting it too reached ~0.96× C: the lever is a miss-blind use class in the word-storage census.",
