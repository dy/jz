...
const WASM_TODO = {
  ...
  raytrace: 'needs data-dependent loop autovectorization of the 8-sphere intersection loop (clang→wasm does). NB: the unroll-bloat hypothesis was DISPROVEN by experiment — forcing the trip-8 sphere loop to stay a loop (vs the current full unroll) measured SLOWER (3.19 vs 3.03 ms), so do not re-chase a body-cost unroll guard here.',
