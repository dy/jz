...
export const indexOf = (rw, args) => {
  ...
  if (searchVal.type === 'string') {
    ...
    return wat(`(local.set ${str} ${rw})
      ...
      (local.set ${len} ${strLen(gc, `(local.get ${str})`)})
      (local.set ${searchLen} ${strLen(gc, `(local.get ${search})`)})
          ...
          (then
            ...
            (block $done_${id} (loop $loop_${id}
              ...
              (block $inner_done_${id} (loop $inner_loop_${id}
                ...
                (if (i32.ne ${strCharAt(gc, `(local.get ${str})`, `(i32.add (local.get ${idx}) (local.get ${j}))`)} ${strCharAt(gc, `(local.get ${search})`, `(local.get ${j})`)})
      ...
      (if (i32.gt_s (local.get ${searchLen}) (local.get ${len}))
        ...
        (else
          ...
          (block $done_${id} (loop $loop_${id}
            ...
            (if (i32.ne ${strCharAt(gc, `(local.get ${str})`, `(local.get ${idx})`)} ${strCharAt(gc, `(local.get ${search})`, `(local.get ${idx})`)})
    ...
    return wat(`(local.set ${str} ${rw})
      ...
      (if (i32.gt_s (local.get ${searchLen}) (local.get ${len}))
        ...
        (else
          ...
          (block $done_${id} (loop $loop_${id}
            (br_if $done_${id} (i32.ge_s (local.get ${idx}) (local.get ${searchLen})))
            (if (i32.ne ${strCharAt(gc, `(local.get ${str})`, `(i32.add (local.get ${offset}) (local.get ${idx}))`)} ${strCharAt(gc, `(local.get ${search})`, `(local.get ${idx})`)})
              (then (local.set ${result} (i32.const 0)) (br $done_${id})))
            (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
            (br $loop_${id})))))
      (local.get ${result})`, 'i32')
...
export const split = (rw, args) => {
  ...
  if (sepVal.type === 'string') {
    if (gc) ctx.usedRefArrayType = true
    ...
    ctx.addLocal(start.slice(1), 'i32')
    ctx.addLocal(result.slice(1), gc ? 'refarray' : 'f64')
    ctx.addLocal(j.slice(1), 'i32')
    ctx.addLocal(match.slice(1), 'i32')
    ...
    const arrNew = gc ? `(array.new $anyarray (ref.null none) (local.get ${count}))` : `(call $__alloc (i32.const ${PTR_TYPE.REF_ARRAY}) (local.get ${count}))`
    const arrSet = gc
      ? `(array.set $anyarray (local.get ${result}) (local.get ${arrIdx}) (local.get ${part}))`
    ...
    return wat(`(local.set ${str} ${rw})
      ...
      (local.set ${len} ${strLen(gc, `(local.get ${str})`)})
      (local.set ${sepLen} ${strLen(gc, `(local.get ${sep})`)})
      ;; Count parts first
        ...
        (else
          ...
          (block $cnt_done_${id} (loop $cnt_loop_${id}
            ...
            (block $m_done_${id} (loop $m_loop_${id}
              ...
              (if (i32.ne ${strCharAt(gc, `(local.get ${str})`, `(i32.add (local.get ${idx}) (local.get ${j}))`)} ${strCharAt(gc, `(local.get ${sep})`, `(local.get ${j})`)})
      ...
      (if (i32.eqz (local.get ${sepLen}))
        (then
          ...
          (block $char_done_${id} (loop $char_loop_${id}
            ...
            (local.set ${part} ${strNew(gc, '(i32.const 1)')})
            ${strSetChar(gc, `(local.get ${part})`, '(i32.const 0)', strCharAt(gc, `(local.get ${str})`, `(local.get ${idx})`))}
            ${arrSet}
        ...
        (else
          ...
          (block $fill_done_${id} (loop $fill_loop_${id}
            ...
            (if (local.get ${match})
              (then
                ...
                (local.set ${part} ${strNew(gc, `(local.get ${partLen})`)})
                (local.set ${k} (i32.const 0))
                (block $cpy_done_${id} (loop $cpy_loop_${id}
                  (br_if $cpy_done_${id} (i32.ge_s (local.get ${k}) (local.get ${partLen})))
                  ${strSetChar(gc, `(local.get ${part})`, `(local.get ${k})`, strCharAt(gc, `(local.get ${str})`, `(i32.add (local.get ${start}) (local.get ${k}))`))}
          ...
          (block $last_done_${id} (loop $last_loop_${id}
            ...
            (br $last_loop_${id})))
          ${arrSet}))
      (local.get ${result})`, gc ? 'refarray' : 'f64', { uniformType: 'string' })
  }
  ...
  if (sepVal.type === 'i32' || sepVal.type === 'f64') {
    ...
    return wat(`(local.set ${str} ${rw})
      ...
      (block $cnt_done_${id} (loop $cnt_loop_${id}
        ...
        (if (i32.eq ${strCharAt(gc, `(local.get ${str})`, `(local.get ${idx})`)} (local.get ${sepChar}))
...
export const replace = (rw, args) => {
  ...
  if (searchVal.type === 'string' && replaceVal.type === 'string') {
    ...
    return wat(`(local.set ${str} ${rw})
      ...
      (local.set ${len} ${strLen(gc, `(local.get ${str})`)})
      (local.set ${searchLen} ${strLen(gc, `(local.get ${search})`)})
      (local.set ${replLen} ${strLen(gc, `(local.get ${repl})`)})
      ...
      (if (i32.lt_s (local.get ${foundIdx}) (i32.const 0))
        ...
        (else
          ...
          (block $b_done_${id} (loop $b_loop_${id}
            ...
            ${strSetChar(gc, `(local.get ${result})`, `(local.get ${k})`, strCharAt(gc, `(local.get ${str})`, `(local.get ${k})`))}
            (local.set ${k} (i32.add (local.get ${k}) (i32.const 1)))
            (br $b_loop_${id})))
          ;; Copy replacement
          (local.set ${k} (i32.const 0))
          (block $r_done_${id} (loop $r_loop_${id}
            ...
            ${strSetChar(gc, `(local.get ${result})`, `(i32.add (local.get ${foundIdx}) (local.get ${k}))`, strCharAt(gc, `(local.get ${repl})`, `(local.get ${k})`))}
            (local.set ${k} (i32.add (local.get ${k}) (i32.const 1)))
            (br $r_loop_${id})))
          ;; Copy after
          ...
          (block $a_done_${id} (loop $a_loop_${id}
            ...
            ${strSetChar(gc, `(local.get ${result})`, `(i32.add (i32.add (local.get ${foundIdx}) (local.get ${replLen})) (local.get ${k}))`, strCharAt(gc, `(local.get ${str})`, `(local.get ${idx})`))}
            (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
            (local.set ${k} (i32.add (local.get ${k}) (i32.const 1)))
            (br $a_loop_${id})))))
      (local.get ${result})`, 'string')
  ...
  if ((searchVal.type === 'i32' || searchVal.type === 'f64') && replaceVal.type === 'string') {
    ...
    return wat(`(local.set ${str} ${rw})
      ...
      (block $find_done_${id} (loop $find_loop_${id}
        ...
        (if (i32.eq ${strCharAt(gc, `(local.get ${str})`, `(local.get ${idx})`)} (local.get ${searchChar}))
          (then (local.set ${foundIdx} (local.get ${idx})) (br $find_done_${id})))
        (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
        (br $find_loop_${id})))
      ;; Build result
...
export const substr = (rw, args) => {
  ...
  return wat(`(local.set ${str} ${rw})
    ...
    (if (i32.lt_s (local.get ${subLen}) (i32.const 0)) (then (local.set ${subLen} (i32.const 0))))
    (local.set ${result} ${strNew(gc, `(local.get ${subLen})`)})
    (local.set ${idx} (i32.const 0))
    (block $done_${id} (loop $loop_${id}
...
export const trimEnd = (rw, args) => {
  ...
  return wat(`(local.set ${str} ${rw})
    ...
    (block $end_done_${id} (loop $end_loop_${id}
      ...
      (local.set ${end} (i32.sub (local.get ${end}) (i32.const 1)))
      (br $end_loop_${id})))
    (local.set ${result} ${strNew(gc, `(local.get ${end})`)})
    (local.set ${idx} (i32.const 0))
    (block $done_${id} (loop $loop_${id}
      (br_if $done_${id} (i32.ge_s (local.get ${idx}) (local.get ${end})))
      ${strSetChar(gc, `(local.get ${result})`, `(local.get ${idx})`, strCharAt(gc, `(local.get ${str})`, `(local.get ${idx})`))}
      (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
      (br $loop_${id})))
    (local.get ${result})`, 'string')
}
  ...
  return wat(`(local.set ${str} ${rw})
    ...
    (block $done_${id} (loop $loop_${id}
      ...
      (block $inner_done_${id} (loop $inner_loop_${id}
        ...
        ${strSetChar(gc, `(local.get ${result})`, `(local.get ${idx})`, strCharAt(gc, `(local.get ${str})`, `(local.get ${srcIdx})`))}
        (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
...
export const padStart = (rw, args) => {
  ...
  if (padArg && padArg.type === 'string') {
    ...
    return wat(`(local.set ${str} ${rw})
      ...
      (local.set ${padStrLen} ${strLen(gc, `(local.get ${padStr})`)})
      (if (i32.or (i32.le_s (local.get ${targetLen}) (local.get ${len})) (i32.eqz (local.get ${padStrLen})))
        ...
        (else
          ...
          (local.set ${result} ${strNew(gc, `(local.get ${targetLen})`)})
          ...
          (block $pad_done_${id} (loop $pad_loop_${id}
            ...
            ${strSetChar(gc, `(local.get ${result})`, `(local.get ${idx})`, strCharAt(gc, `(local.get ${padStr})`, `(local.get ${padIdx})`))}
            (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
          ...
          (block $copy_done_${id} (loop $copy_loop_${id}
            ...
            ${strSetChar(gc, `(local.get ${result})`, `(i32.add (local.get ${padLen}) (local.get ${idx}))`, strCharAt(gc, `(local.get ${str})`, `(local.get ${idx})`))}
            (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
            (br $copy_loop_${id})))))
      (local.get ${result})`, 'string')
  }
  ...
  return wat(`(local.set ${str} ${rw})
    ...
    (if (i32.le_s (local.get ${targetLen}) (local.get ${len}))
      ...
      (else
        ...
        (block $pad_done_${id} (loop $pad_loop_${id}
          (br_if $pad_done_${id} (i32.ge_s (local.get ${idx}) (local.get ${padLen})))
          ${strSetChar(gc, `(local.get ${result})`, `(local.get ${idx})`, `(local.get ${padChar})`)}
          (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
          (br $pad_loop_${id})))
        ;; Copy original
        (local.set ${idx} (i32.const 0))
    ...
    return wat(`(local.set ${str} ${rw})
      ...
      (if (i32.or (i32.le_s (local.get ${targetLen}) (local.get ${len})) (i32.eqz (local.get ${padStrLen})))
        ...
        (else
          ...
          (block $pad_done_${id} (loop $pad_loop_${id}
            (br_if $pad_done_${id} (i32.ge_s (local.get ${idx}) (local.get ${padLen})))
            ${strSetChar(gc, `(local.get ${result})`, `(i32.add (local.get ${len}) (local.get ${idx}))`, strCharAt(gc, `(local.get ${padStr})`, `(local.get ${padIdx})`))}
            (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
            (local.set ${padIdx} (i32.add (local.get ${padIdx}) (i32.const 1)))
            (if (i32.ge_s (local.get ${padIdx}) (local.get ${padStrLen}))
              (then (local.set ${padIdx} (i32.const 0))))
  ...
  return wat(`(local.set ${str} ${rw})
    ...
    (if (i32.le_s (local.get ${targetLen}) (local.get ${len}))
      ...
      (else
        ...
        (block $pad_done_${id} (loop $pad_loop_${id}
          (br_if $pad_done_${id} (i32.ge_s (local.get ${idx}) (local.get ${padLen})))
          ${strSetChar(gc, `(local.get ${result})`, `(i32.add (local.get ${len}) (local.get ${idx}))`, `(local.get ${padChar})`)}
          (local.set ${idx} (i32.add (local.get ${idx}) (i32.const 1)))
          (br $pad_loop_${id})))))
    (local.get ${result})`, 'string')
}
