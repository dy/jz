...
import { PTR_TYPE, tv, fmtNum, asF64, asI32, truthy, conciliate, typeOf, isF64, isI32, isString, isArray, isObject, isClosure, isRef, isRefArray, bothI32, isHeapRef, hasSchema } from './types.js'
...
import { nullRef, mkString, envGet, envSet, arrGet, arrGetTv, arrLen, objGet, strCharAt, mkArrayLiteralTv, callClosure } from './gc.js'
...
function genIdent(name) {
  ...
  if (ctx.hoistedVars && name in ctx.hoistedVars) {
    ...
    return tv(type, envGet(opts.gc, ctx.ownEnvType, '$__ownenv', index, type))
  }
  ...
  if (ctx.capturedVars && name in ctx.capturedVars) {
    ...
    return tv(type, envGet(opts.gc, ctx.currentEnv, envVar, index, type))
  }
...
function genClosureCall(name, args) {
  ...
  return tv('f64', callClosure(ctx, opts.gc, closureVal[1], argWats, args.length, isNullable))
}
...
function genClosureCallExpr(closureVal, args) {
  ...
  const wat = callClosure(ctx, opts.gc, closureVal[1], argWats, args.length, true, returnsClosure)
  return tv(resultType, wat, returnsClosure ? { closureDepth: remainingDepth - 1 } : undefined)
}
...
function genAssign(target, value, returnValue) {
  ...
  if (ctx.hoistedVars && target in ctx.hoistedVars) {
    ...
    const setCode = envSet(opts.gc, ctx.ownEnvType, '$__ownenv', index, asF64(val)[1])
    const getCode = envGet(opts.gc, ctx.ownEnvType, '$__ownenv', index, type)
    return returnValue
      ? tv(type, `(block (result f64) ${setCode} ${getCode})`)
      : setCode + '\n    '
  }
  ...
  if (ctx.capturedVars && target in ctx.capturedVars) {
    ...
    const envVar = opts.gc ? '$__envcast' : '$__env'
    const setCode = envSet(opts.gc, ctx.currentEnv, envVar, index, asF64(val)[1])
