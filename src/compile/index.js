...
function analyzeFuncForEmit(func, programFacts) {
  ...
  if (_reps) {
    for (const [k, r] of _reps) {
      ...
      if (process.env.JZ_DBG_PARAMVAL2 && name.includes('uleb')) console.error(JSON.stringify({ name, pname, k, val: r.val, reassigned, trustworthy: paramValTrustworthy(r), possibleKinds: r.possibleKinds ? [...r.possibleKinds] : r.possibleKinds, kindsCoverage: r.kindsCoverage }))
