 ...
 * setLinkDemand() tripwire), `inspect`, `warnings`. `error` gets no view
 * here — its only reader is ctx.js's own err() (internal); nothing outside
 * ctx.js ever reads ctx.error as data, so there is no external read
 * contract to make structural.
 *
 ...
 *
 * Construct FRESH at every call site, never memoize across a reset():
 * ctx.memory and ctx.linkDemand are REPLACED (new object identity), not
 * mutated in place, by every reset() (ctx.js: `ctx.memory = {…}`,
 * `ctx.linkDemand = {…}`) — a view snapshot taken once at module load would
 * read a stale, pre-reset object for the rest of the process.
 *
 * READ-ONLY: nothing here mutates a subtree. Writes keep their existing path
 * — setLinkDemand() where a tripwire exists, plain assignment where the
 * subtree is already write-once-at-reset or single-owner (memory, inspect,
 * warnings — survey §2).
 *
 ...
 *  stands alone rather than folding into assembleView, and gets no phase
 *  assertion (no single sessionPhase value covers all its readers). */
