/**
 * Nest-level typed-array loop-versioning: lifts a whole nest of loops behind
 * ONE top-entry guard (so the outer-strip / per-pixel / iterated-reduce
 * recognizers see a bare nest in the fast arm) when every nested level's
 * bound is provably static at the top. Built strictly on top of the
 * single-loop scan in `loop-versioning.js` (`versionableTypedFor`) — a
 * one-directional dependency, never called back.
 *
