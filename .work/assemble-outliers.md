...
restart loses no analysis — same discipline as `.work/analyze-traversals.md`
...
- **`stripStaticDataPrefix`**'s `shift()` closure — already bounded
  (`prefix <= off < buf.length`) this session per `.work/region-release-
...
- **`finalizeClosureTable`** — already lean and heavily doc-commented with
  hard-won invariants (`.work/region-release-notes.md`'s Class-1 sessions);
