...
- `module/typedarray.js`'s BigInt64Array/BigUint64Array element-store path (the `isBigInt` branch of `.typed:[]=`, three call sites) was the one hunk not yet on main: still an unconditional `i64.reinterpret_f64` where its three sibling chokepoints already use the runtime tag-checked `maybeUnboxBigInt`. Could not construct a failing native repro against main's own (call-target-index-based) architecture through any ordinary-program shape tried, mirroring the branch's own described trigger (a box-forcing Number|BigInt union / ternary-with-a-`.`-member-call-arm feeding a BigInt64Array store) — consistent with the branch's own trigger being specific to its now-retired Tier-1/Tier-2 materialization, not to call-target-index.js. Ported anyway, same reasoning the readI64 hardening above already used ("defense-in-depth, matching established practice... the same order-sensitivity... can apply anywhere this fixpoint's verdict gets consumed"). Pinned in test/data.js ("range-boundary family survives a BigInt64Array element store"): the statement-position store round-trips correctly for the full boundary family; the non-void/value-position call site is confirmed patched by direct WAT inspection (`call $__ptr_type` present) rather than a round-trip, since a value-position typed-array assignment expression's own result hits a SEPARATE, also pre-existing (confirmed identical on unmodified main), unrelated representation-tracking gap — out of this fix's scope, not touched, not this fix's regression.
...
Battery (fix/shape8-pins worktree): test/data.js green, test/index.js 3828/3829 (1 pre-existing skip), `npm run build` clean, kernel-parity 33/33, kernel-oracle 14/14, `JZ_TEST_TARGET=jz.wasm node test/index.js` green, refactor-oracle 552/560 identical, 8 attributed (`bench:watr`/`watr:watr.js` at all four optimize levels, +44 to +200 bytes each — the typedarray.js hardening above, confirmed reachable on watr's own real source, the exact corpus the original branch found this hazard against). Nothing left unlanded; fix/shape8-member-callee is safe to delete.
