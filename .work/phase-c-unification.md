...
Battery (fix/shape8-pins worktree): test/data.js green, test/index.js 3828/3829 (1 pre-existing skip), `npm run build` clean, kernel-parity 33/33, kernel-oracle 14/14, `JZ_TEST_TARGET=jz.wasm node test/index.js` green, refactor-oracle attributed (see that check's own output for the one intentional src diff — the typedarray.js hardening above — everything else 560/560 identical). Nothing left unlanded; fix/shape8-member-callee is safe to delete.
