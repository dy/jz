...
`.work/archive/porffor-alpha3-audit.md` (2026-08-27, one day old, same architecture)
...
```
`.work/archive/handoff-2026-08-22.md` (2 days earlier) counted **10** files over 3k
...
- `src/kind.js` — 27 lines, pure re-export, header cites `.work/archive/kind-split.md`
- `src/type.js` — 71 lines (mostly a load-bearing doc comment, see §2c),
  cites `.work/archive/type-split.md`
- `src/compile/program-facts.js` — 86 lines, cites `.work/archive/program-facts-split.md`
- `src/compile/representation-plan.js` — 62 lines, cites `.work/archive/representation-plan-split.md`
- `src/compile/analyze.js` — 55 lines, cites `.work/archive/analyze-traversals.md`
...
lines, matches the handoff's "~1.3k" exactly, and `.work/archive/todo.md:8512` already
...
*third*, independent bounded fixpoint (≤64 rounds, `.work/archive/program-facts-split.md`
...
| — | function-signature narrowing | `src/compile/narrow.js` phases D-J | Per-call-site-census specialized param/result reps | a later, more precise LAYER on top of #1/#9 — both stay live simultaneously (.work/archive/program-facts-split.md §7.2: "no reader was found reading either fact before its relevant producer settled" — sound only by pass-ordering discipline) |
...
what `.work/archive/porffor-alpha3-audit.md` measured for the full 344 s self-build
...
nodes) peaks at 251 MB during C-emission (`.work/archive/porffor-alpha3-audit.md`) —
...
89% full at audit time). Citing `.work/archive/porffor-alpha3-audit.md`
...
`.schemaSid` are named directly in `.work/archive/porffor-alpha3-audit.md`'s own
...
they build — `.work/archive/porffor-alpha3-audit.md` §1-2) is the right comparison
...
`.work/archive/program-facts-split.md` §7 ("Freeze audit") is the clearest first-party
...
(`.work/archive/refactor-oracle.md`): excludes the self-host compile *by default*
...
`.work/archive/refactor-oracle.md` states its own limits plainly — "cannot prove... runtime
...
in `.work/archive/handoff-2026-08-22.md` ("Numeric use called
...
backlog (`.work/archive/todo.md:8512`, "extract inferTypedValueRanges' range algebra
...
obviously wrong — `.work/archive/porffor-alpha3-audit.md`'s own "what not to copy"
...
| 2 | Optimize-while-constructing, no post-hoc optimizer | **Adapt, not adopt whole** | jz cannot drop its optimizer — direct Wasm quality is the stated product claim (`.work/archive/porffor-alpha3-audit.md`'s own "what not to copy" list agrees). The *transferable* half — fold obvious garbage (dead conversions, constant chains) at IR-construction time, before any pass has to rediscover it — is cheap and compatible with keeping a real optimizer; adopt that half only. |
...
per `.work/archive/handoff-2026-08-22.md`, JZ already leads Porffor-native on 43/43
