...
.work/evidence.md §"Body-write-only BigInt params" (the probes) and §"RETIREMENT
...
see .work/archive/printer-trio.md: date.js flat .valueOf overwrite, same root as
...
## Self-host fixpoint divergence — investigation trail (relocated from .work/archive/todo.md, 2026-08-27)
...
**CLOSED 2026-08-29 (`fix/wrong-values`).** The unconditional attempt asked the wrong question at every store. The direct BigInt element-store path now delegates to `readI64(val, emitted)`, the existing RepresentationPlan-aware read authority: only a statically materialized RHS gets the guarded PTR.BIGINT unbox; a proven-raw RHS keeps the old reinterpret path. Temp reads are explicitly typed f64 so `readI64` cannot numerically convert their bits. The pin now requires exact values for both BigInt64Array and BigUint64Array at O0/O2/O3. A shared native/kernel corpus pairs a boxed `0x7ffa8000ffffffffn` payload with the identical proven-raw payload: the first must unbox, while the second must emit no `$__ptr_type` call (an unconditional check would dereference near 4 GiB). A materialized Number arm now throws `TypeError` before either an in-range or OOB store; statically non-BigInt inputs and source-level `try/catch` around this compact code-error path reject at compile time rather than acquire different conversion/catch semantics. Full native and kernel-target suites, kernel parity/oracle, build, and the refactor oracle all cover the pair.
