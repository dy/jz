...
(rounds 1–6, deleted round-3 design doc — history in `.work/archive/todo.md`
...
history in `.work/archive/todo.md` §deletion-sweep). It does not re-open the
...
doc's §19 "documented gap" row, history in `.work/archive/todo.md` §deletion-sweep)
...
class, not a hypothetical.** `.work/archive/todo.md`'s tail entry (2026-08-05/06,
...
- **Round 1 (always-box)**: correct, warm-blocked — measured
  ...
  history in `.work/archive/todo.md` §deletion-sweep).
...
- **Round 3 (solver-owned `bigintBoxed`, raw iff proven)**: the SOUND
  ...
  PARKED it: "keep that limitation" (ledger reference, `.work/archive/todo.md`
  "ROUND 2 WALL" entries).
- **The 84-site BigInt-scrub analysis** (commit-era 4bcb8bc0, `.work/archive/
...
- **The presentVal design** (deleted doc, history in `.work/archive/todo.md`
...
| 13 | Deleted presentVal design's §19 documented bitwise `0n`-vs-`0` gap (history in `.work/archive/todo.md` §deletion-sweep) | Closes (both operands are tag-checkable, no carrier-bits collision) |
...
`.work/archive/carrier-box-baseline.md` (repro command + result table + one finding).
...
- `carrierF64` (`src/ir.js`) — the design's own single W-sink choke-point
  for boxed-value storage positions (confirmed from `.work/archive/todo.md`'s
...
Lands ON the heap-kind registry (`.work/archive/heap-kind-registry-design.md`,
...
5. `__sclone_rec` (module/collection.js) — BIGINT joins the ATOM/STRING
   ...
   program's own re-enable path (`.work/archive/region-arena-design.md`), per the
...
pointers.js (the pin), .work/archive/carrier-representation-design.md (this
...
`test/fixtures/carrier-layout-repro.js` (the pin), `.work/archive/carrier-
...
`test/pointers.js` (the pin flip), `.work/archive/carrier-representation-
...
(2026-08-07, .work/archive/todo.md "MAP.GET KIND PROMOTION" seed, §17's own named
lever)
...
and the matching `.work/archive/todo.md` status update are new, committed
separately, plain messages, no push.


## §19. PROPERTY-KIND TRACING scope check — WALL CONFIRMED, no implementation
attempted (2026-08-07, .work/archive/todo.md "PROPERTY-KIND TRACING — coordinator
...
`.work/archive/todo.md` status update are new, committed separately, plain
messages, no push.
...
`.work/archive/todo.md` status update commit separately, plain messages, no push.
...
only). This ledger entry + the matching `.work/archive/todo.md` status update
...
This ledger entry + `.work/archive/todo.md` status update commit separately.
...
KIND_UNIVERSE/TOP instead of a null sentinel) — `.work/archive/lattice-design.md`'s
...
structurally opt-in per the COORDINATOR RULING on OQ1** (`.work/archive/
.work/archive/lattice-design.md`: census-derived kind unions surface ONLY through
...
for `slotVT`'s callers (a real soundness boundary, `.work/archive/lattice-design.md`
...
matching `.work/archive/todo.md`/`.work/archive/lattice-design.md` status updates commit
separately, plain messages, no push.
...
— banked, no flip (2026-08-09, `.work/archive/context-sensitivity-survey.md`'s
...
(the pin). This ledger entry + `.work/archive/todo.md`'s matching status update
commit separately, plain messages, no push.

## §25. AUDIT-#17's unification hypothesis vs this §24 UNDEF_NAN gap —
TESTED, ANSWERED NO (2026-08-09, `.work/archive/todo.md`'s own matching entry has
...
module-scope nested-object-literal store-loop miscompile (`.work/archive/todo.md`,
...
separate, ALSO-still-open hunt (`.work/archive/todo.md`) — do not spend a future
...
`.work/archive/todo.md`'s own entry for this session, not duplicated here.
...
the shared tree's own default `dist/jz.wasm`. `.work/archive/todo.md`'s own
...
entry (`.work/archive/carrier-representation-design.md` only) is this session's
one commit.
...
**Local commits.** This ledger entry (`.work/archive/carrier-representation-design.md`
...
instrument, new), this ledger entry (`.work/archive/carrier-representation-
...
2 stale-assertion fixes, item 5). This ledger entry + `.work/archive/todo.md`
...
CONSERVATIVE PAIRING (commit `83c7f9bc`, .work/archive/carrier-representation-
...
+ `.work/archive/todo.md` status update filed separately.
...
entry + `.work/archive/todo.md` status update filed separately.
...
`test/pointers.js`; (3) this ledger entry plus `.work/archive/todo.md`'s
goal-state update.
...
BLOCKED, joining the pre-flip blocker .work/evidence.md already named (item 4's
