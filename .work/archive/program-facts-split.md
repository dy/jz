...
prior art already read, not re-derived here: `.work/archive/refactor-oracle.md` (the
gate), and `.work/archive/analyze-traversals.md` (`refactor/analyze-traversals`
...
- **`walkAst`/`some` (src/ast.js) retirement**: every hand-rolled walk in this
  ...
  riskier change" `.work/archive/walk-count-design.md` already declined for the
  sibling file's analogous cases. None of this file's walks are the
  iterative-by-necessity or fixpoint-outer-loop shape `.work/archive/analyze-traversals.md`
...
convention (§1's dead-import note, `.work/archive/analyze-traversals.md`'s "both readings
