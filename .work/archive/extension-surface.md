...
the `test262` line in .work/archive/todo.md ("know every fail by face — jzify or error cleanly, never fail
...
| 0.21 | **for-of over nullish** | silently 0 iterations (JS: TypeError) → deliberate decision: throw or documented-permissive | surfaced independently by the instrumented-kernel session (.work/archive/todo.md) — it masked two real roots there; leaning THROW |
...
into the closure env (mutable captures shipped — .work/evidence.md Closures), body becomes a
...
(5, 12–15 are free — .work/evidence.md's "4 free" undercounts; SSO moved to an aux bit) + a
