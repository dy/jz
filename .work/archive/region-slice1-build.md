...
Implements `.work/archive/region-arena-design.md` Slice 1 (fixpoint-round region) per
the corrected arithmetic at `ad5e9e1c` and `.work/archive/region-slice1-liveness.md`'s
...
- A dedicated micro-kernel (`.work/region-diff-entry.mjs`, mirroring
  `.work/archive/watr-diff-entry.mjs`'s method) compiling `watr`'s
  ...
  - A genuine jz-optimizer bug was found and is NOT fixed: at micro-kernel
    ...
    surfaced — worth a `.work/archive/todo.md` line of its own (below).
