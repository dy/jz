...
`.work/archive/phase-c-unification.md` — and the i64Hex hazard family), nullish
...
| bigint | `src/ir/bigint.js` | `bigintStrict`, `bigintEraseErr`, `boxBigInt`, `unboxBigInt`, `applyBigintRepresentationAction`, `maybeUnboxBigInt`, `isSchemaSlotBigintPossible`, `isTernaryBoxedBigint`, `isPlanTaggedBigint`, `readI64` — the phase-C box/unbox pairing family. Moved **verbatim, same relative order, same conditional structure** — this is the file `.work/archive/phase-c-unification.md` names as load-bearing (`readI64`'s `maybeUnboxBigInt` vs `unboxBigInt` dispatch, the i64Hex hazard notes). No logic touched. |
...
- `isPureIR` — recursive **AND-fold** short-circuiting at the ROOT first
  ...
  `.work/archive/analyze-traversals.md` already used to decline several
