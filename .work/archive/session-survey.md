...
`dyn-closure-tables.js` all landed/changed post-audit-#13 per .work/evidence.md
...
| `features` | compile | prepare (`setFeature()` only) | compile, optimizer, stdlib factories | **already phase-disciplined** — the FeaturePlan freeze (landed, .work/evidence.md §FeaturePlan freeze). `setFeature()` throws under `JZ_DEBUG_INVARIANTS` if called after `post-analyze`. THIS IS THE PRECEDENT the coordinator should generalize, not re-derive. |
...
.work/evidence.md §FeaturePlan freeze) is the load-bearing cost precedent: its
...
**(d) The full `CompileSession` record** (program, analyses, passes, plans,
   target — per .work/evidence.md's Stage-4 framing). Highest value (closes
   ...
   first and the BindingId/FunctionPlan precedents (.work/evidence.md
...
[`.work/archive/compile-session-func-survey.md`](compile-session-func-survey.md).
