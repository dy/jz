...
`d7c7e67f` ".work/archive/kind-split.md: record the main-merge + final battery results" merged
...
| `test` (opt0) | `npm run test:opt0` | `JZ_TEST_OPTIMIZE=0 node test/index.js` | not run | Same 95-file suite at a different optimize tier — identical code path to the default leg, ~10 more minutes for a second full pass. Not independently re-run this session; last-known-green from `.work/archive/handoff-2026-08-22.md`'s 2026-08-26 entry: `matrix default/O0/O3 3660/3659/0/1`. No fresh confirmation at `a15ec98c`. |
...
through `.work/archive/handoff-2026-08-22.md` and `.work/archive/v1-architecture-campaign.md`
...
Cross-check against `.work/archive/porffor-alpha3-audit.md:19` (2026-08-27, JZ ref
...
cited from `.work/archive/porffor-alpha3-audit.md:20-26`, 2026-08-27, JZ ref
`4c38662f`):**
...
Porffor's target isn't wasm — `.work/archive/porffor-alpha3-audit.md:9-33`): Porffor
...
header). Historical citation (2026-08-26, `.work/archive/handoff-2026-08-22.md`):
...
(`.work/archive/handoff-2026-08-22.md:463-495`) — Gate 1 (Soundness, RED) named two
...
`.work/archive/v1-architecture-campaign.md:217-219` ("reject the exact 137
...
|---|---|---|---|---|
| 1 | Full recursive jz×jz self-compile below wasm32's 4 GiB | **NONE** — no CI job runs the 162-module probe at all; `.work/`-tracked hand diagnostic only | **NOT MET** — traps at exactly 2^32 bytes on every measurement in the campaign's history (cited, not re-run) | Engine work: streaming/compact encoder or region-scoped release (both named, neither landed — `.work/archive/v1-architecture-campaign.md`'s Slice 6 + `.work/archive/porffor-alpha3-audit.md`'s ranked P0 list) |
| 2 | Once self-compile succeeds, match/beat Porffor's self-host wall+RSS | **NONE** (documented only in `CONTRIBUTING.md:158-163`) | **NOT MET / NOT MEASURABLE** — can't start until #1 closes; today's *one-level* hosted build is already ~1.5–1.7× slower, ~2.2–2.3× heavier than Porffor's full native self-build | Blocked on #1, then engine work per the 8 ranked gaps in `.work/archive/porffor-alpha3-audit.md` |
