...
`index.js` (test/scripts excluded, same scope as .work/archive/session-survey.md),
...
.work/archive/session-survey.md made for its own census.

## 0. Headline finding — `ctx.func` is two subtrees wearing one name

.work/archive/session-survey.md §5 already caught this exact shape for `ctx.transform`
...
FINDING: .work/archive/session-survey.md's own §1 aggregate table cites **410 write-sites
...
`compile/closure-plan.js` (ClosureEnvPlan, landed per .work/archive/closure-plan-design.md
...
`refinements`, `localProps`. **This is the bucket .work/archive/session-survey.md's §2
...
the DBG-enforced freeze .work/evidence.md's Stage-2 entry already names
...
for `func` specifically (distinct from .work/archive/session-survey.md §3's
...
driver, per .work/archive/session-survey.md §5's ruling-adopted framing: "cost is
proportional to write-site COUNT, not subtree complexity") — modulated by
read-frequency risk where .work/archive/session-survey.md's slice-(c) AS-LANDED report
...
The remaining 198 (43%) — AnalysisFacts alone — is where .work/archive/session-survey.md's
...
FunctionIdentity (which .work/archive/session-survey.md's general framing might suggest
...
- **Direct existing precedent already in the codebase**, not a new pattern
  ...
  Self-compile subset-safe by the same precedent (.work/archive/session-survey.md §4:
...
- **Real payoff, not just cleanup**: directly generalizes .work/evidence.md's own
