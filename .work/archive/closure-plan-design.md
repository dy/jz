...
`.work/archive/todo.md` "AUDIT-#18 DOWNSTREAM GATE LADDER" (line 57) — the session
...
`.work/archive/lattice-design.md`'s own precedent for this phrasing.
...
measurement (`.work/archive/todo.md:95`) generalized to a fresh, independent scan.
...
corpus to re-derive concrete paying shapes from," `.work/archive/todo.md:130-142`)
...
The entire audit-#17→#18 saga (`compile/index.js:410-463`, `.work/archive/
.work/archive/todo.md`'s AUDIT-#17/#18 entries) was about proving increasingly narrow
...
1. **Introduce `astClosurePlan` + `mintClosureEnvPlans`, storage values
   ...
   (`.work/archive/todo.md:484-529`, a same-shape "plan minted earlier, same
...
4. **Retire the static path** — delete `module/function.js`'s
   ...
   grant costs nothing (dist even shrank, `.work/archive/todo.md:96`); this slice
...
5. **This session's weak-predicate self.js tag count (371) does not match
   audit-#18's own reported pre-fix grant count (626, `.work/archive/todo.md:95`)**
...
**Local commit**: `.work/archive/closure-plan-design.md` only. No `src/` changes —
...
src/compile/emit.js, module/function.js) + this entry + `.work/archive/todo.md`'s
matching append.
