...
Design pass for `.work/archive/todo.md` "PRODUCT-LATTICE CAMPAIGN — design-pass opening
brief" (line 7122), grounded in `.work/archive/lattice-survey.md` (a212fa1f, treated as
ground truth per the brief) and `.work/evidence.md` §Middle-end consolidation
(line 693). Design only — no `src/` changes. Every decision below is argued
against a specific survey finding or a specific wall (`.work/archive/carrier-
...
machinery (.work/evidence.md's induction-variable range work) — this design
...
(.work/archive/carrier-representation-design.md:2019-2068) traced the dominant, banked
...
an integer" — and §22 (.work/archive/carrier-representation-design.md:3086-3119) already
...
| slot* family (`slotTypes`, `slotTypedCtors`, `slotBigintObserved`, `slotObjSids`, `slotIntCertain`/`slotI32Certain`) | Named projections of ONE `SlotFact` per `(sid,idx)`: `slotVT=kindsOf`, `slotTypedCtorBySid=rep`, `slotBigintProvenBySid=repClass.has('boxed')`, `slotObjSids=pointsToSet`, `slotIntCertainAt`/`slotI32CertainAt=numeric.level` | 5 parallel `Map<sid,...[]>` structures → 1 `Map<sid,Fact[]>` — literally the audit-#15 verdict's own words (.work/archive/todo.md:6962, "SlotFact with named projections... replacing the fragmenting slot* census family") |
...
.work/evidence.md:862-871) — only delete `makeValTracker`'s poison-Set AFTER that
...
§17/§22 already used (.work/archive/carrier-representation-design.md:1980-1982, stripped
...
7. **FINDING-5's rename (Slice 0) is a hard sequencing dependency, LOW
   ...
   reason)` entry point (.work/evidence.md:704-708's own target name) collides
...
1. `1db8e55e` → reverted `f8f61591` (ledgered **audit-#7 P0**, 2026-08-02).
   ...
   message and `.work/archive/todo.md`'s 2026-08-02 status entry: (a) **absent
...
3. `79082fb2`→`3782a692` (represented-maybeUndefined: `presentVal` as a
   ...
   — `.work/archive/represented-maybe-undefined-design.md` Slices 1-3, deleted at
...
.work/evidence.md:862-871 describes applies to genuinely-independent twins, e.g.
...
explicitly opted-in chokepoint" architecture (`.work/archive/lattice-design.md`
...
- `JZ_CARRIER_BOX=1` battery: ran in full, twice, on a CLEAN isolated
  ...
  name and `.work/evidence.md` citation — nothing to do with schema slot
...
in `.work/archive/carrier-representation-design.md` §23; this entry is the
...
wall `.work/archive/carrier-representation-design.md` §20/§21 already found and
...
src/compile/narrow.js) + this entry + `.work/archive/todo.md`'s matching Status
append.
