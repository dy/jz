...
Design only — no `src/` changes. Grounded in `.work/archive/session-survey.md` (S5(d)
and its landed slices a/b/c), `.work/archive/ctxfunc-survey.md` (the six-lifetime
`ctx.func` decomposition. The task brief also names
`.work/archive/compile-session-func-survey.md`; this design uses
`.work/archive/ctxfunc-survey.md` for the six-lifetime survey, cross-confirmed against
every citation of it in `.work/evidence.md` §Region-arena's decision-point
...
`.work/archive/ctxfunc-survey.md` (2026-08-12) estimated the full record at "~30-40×
...
landed 2026-08-12, `.work/archive/ctxfunc-survey.md`'s own AS-LANDED log) plus `ea423728`
...
| `prepare/index.js`'s 14 lets, `module/regex.js`'s 4 lets, `optimize/vectorize.js`'s 4 arm/disarm flags | Session-survey's own binding ruling (slice a, `.work/archive/session-survey.md` §"COORDINATOR RULING"): "module-scope perf-motivated lets may stay module-scope — the requirement is single-choreography reset, not relocation." Already `RESET_HOOKS`-registered. Re-opening this ruling is out of scope; flagged as the residual reentrancy gap (§2.3) |
...
`b3cb4f8b`'s ledger entry (`.work/evidence.md` §Region arena, "Decision
...
named in `.work/archive/bigint-retirement-design.md` §6/`.work/archive/heap-epoch-design.md`
...
`.work/archive/session-survey.md`'s slice (a)/(b) "storage-location change, no reader
change" cost model.
...
**Heap-epoch design (`75a9638d`, `.work/archive/heap-epoch-design.md`).** Its own
...
**BigInt retirement Slices 2-5 (`.work/archive/bigint-retirement-design.md` §9;
...
`ctx.types` (the deleted fields are among the 65 `.work/archive/ctxfunc-survey.md`
...
above and `.work/archive/todo.md:10177`).** When it lands, it becomes session field
...
`.work/archive/ctxfunc-survey.md`'s coordinator ruling already forbade ("do not implement
...
registry/frame naming collision (§0's headline finding in `.work/archive/ctxfunc-survey.md`
