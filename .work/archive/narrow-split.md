...
not in the repo) — same method `.work/archive/vectorize-split.md` used, so they
...
2. **`inferTypedValueRanges` is 181 lines (1633–1813), not ~1,270.** The
   ...
   as if only one existed. This isn't a fresh mistake: `.work/archive/pipeline-minimality.md`'s
...
treatment `.work/archive/vectorize-split.md` gives `tryDivergentEscapeVectorize`: named,
...
`.work/archive/vectorize-split.md`'s own scope (it doesn't enumerate per-module imports
either).
...
actual outlier — also `.work/archive/pipeline-minimality.md`'s Queue-item-4 "Files
...
of `narrowSignatures` is its own audit, sized like `.work/archive/vectorize-split.md`'s
...
**Recipe** (house convention for this exact campaign — `.work/archive/refactor-oracle.md`'s
...
3. `node scripts/battery.mjs` — full gate (native/O0/O3/`dbg`=O3+`JZ_DEBUG_INVARIANTS`/wasi/kernel-on-wasm/functional
   ...
   **Known pre-existing failure, not a regression to chase**: per
   `.work/archive/pipeline-minimality.md`'s M1 slice notes, `test/jsstring.js`
...
1. **Concurrent edit, `narrowBoolResults` (2925–2984 in this snapshot).**
   ...
   by `.work/archive/pipeline-minimality.md`'s M1b "No-go regions" list, which
...
2. **Line-number drift — observed, not just theoretical.** Between this
   ...
   byte-identity conversion contract, `.work/archive/pipeline-minimality.md`). A
...
treatment `.work/archive/vectorize-split.md` gave `tryDivergentEscapeVectorize`.

### Dead-code sweep — held-file finding resolution

`.work/archive/dead-exports-sweep.md`'s one narrow.js-specific finding —
