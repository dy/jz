...
Size, speed, memory, and compile-time work tracked as campaigns rather
than one-off findings. Currently: `perf/size-leadership` (below, from
`size-leadership.md`, campaign branch off `8986c2e2`). `perf/self-compile-
memory` is a live branch (see `plan.md`'s Campaign 2026-08-28/29 section)
but has not landed a `.work` note as of this snapshot — nothing to merge
here yet; its eventual notes are a pending fold-in (see `README.md`). The
4 GiB self-compile memory numbers that already exist (allocation-count
attribution, region-arena measurements) live in `plan.md`'s "4 GiB
self-compile" section and `evidence.md`'s dated entries, not duplicated
here.
...
stratified — the real monolith is `__dyn_set`/`__dyn_get_t`, whose
...
`local.tee` reuse") is closed everywhere it recurs, not just here, but was
never by itself the lever that flips this one case green. Remaining ~340B
