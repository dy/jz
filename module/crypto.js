...
export default (ctx) => {
  ...
  wat('__get_random_values', `(func $__get_random_values (param $p i64) (result f64)
    ...
    (if (i32.ne (call $__ptr_type (local.get $p)) (i32.const ${PTR.TYPED}))
      (then (global.set $__jz_last_err_bits (i64.reinterpret_f64 (f64.const ${ERR.CRYPTO_NOT_TYPED}))) (throw $__jz_err (f64.const ${ERR.CRYPTO_NOT_TYPED}))))
    ...
    (if (i32.and
      ...
      (then (global.set $__jz_last_err_bits (i64.reinterpret_f64 (f64.const ${ERR.CRYPTO_FLOAT_TYPE}))) (throw $__jz_err (f64.const ${ERR.CRYPTO_FLOAT_TYPE}))))
    (local.set $bl (i32.shl (call $__len (local.get $p)) (call $__typed_shift (local.get $et))))
    (if (i32.gt_u (local.get $bl) (i32.const 65536))
      (then (global.set $__jz_last_err_bits (i64.reinterpret_f64 (f64.const ${ERR.CRYPTO_QUOTA}))) (throw $__jz_err (f64.const ${ERR.CRYPTO_QUOTA}))))
