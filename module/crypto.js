...
export default (ctx) => {
  ...
  wat('__get_random_values', `(func $__get_random_values (param $p i64) (result f64)
    ...
    (if (i32.ne (call $__ptr_type (local.get $p)) (i32.const ${PTR.TYPED}))
      (then (global.set $__jz_last_err_bits (i64.reinterpret_f64 (f64.const ${errorCodeLiteral(ERR.CRYPTO_NOT_TYPED)}))) (throw $__jz_err (f64.const ${errorCodeLiteral(ERR.CRYPTO_NOT_TYPED)}))))
    ...
    (if (i32.and
      ...
      (then (global.set $__jz_last_err_bits (i64.reinterpret_f64 (f64.const ${errorCodeLiteral(ERR.CRYPTO_FLOAT_TYPE)}))) (throw $__jz_err (f64.const ${errorCodeLiteral(ERR.CRYPTO_FLOAT_TYPE)}))))
    (local.set $bl (i32.shl (call $__len (local.get $p)) (call $__typed_shift (local.get $et))))
    (if (i32.gt_u (local.get $bl) (i32.const 65536))
      (then (global.set $__jz_last_err_bits (i64.reinterpret_f64 (f64.const ${errorCodeLiteral(ERR.CRYPTO_QUOTA)}))) (throw $__jz_err (f64.const ${errorCodeLiteral(ERR.CRYPTO_QUOTA)}))))
  ...
  wat('__random_uuid', `(func $__random_uuid (result f64)
    ...
    (local.set $base (call $__alloc (i32.const 76)))
    ...
    (block $d (loop $l
      ...
      (i32.store16 (i32.add (local.get $out) (i32.shl (local.get $j) (i32.const 1))) (select (i32.add (local.get $n) (i32.const 87)) (i32.add (local.get $n) (i32.const 48)) (i32.gt_u (local.get $n) (i32.const 9))))
      (local.set $n (i32.and (local.get $b) (i32.const 15)))
      (i32.store16 (i32.add (i32.add (local.get $out) (i32.shl (local.get $j) (i32.const 1))) (i32.shl (i32.const 1) (i32.const 1))) (select (i32.add (local.get $n) (i32.const 87)) (i32.add (local.get $n) (i32.const 48)) (i32.gt_u (local.get $n) (i32.const 9))))
      ...
      (if (i32.or
        ...
        (then
          (i32.store16 (i32.add (local.get $out) (i32.shl (local.get $j) (i32.const 1))) (i32.const 45))
