...
- **Errors are their message strings** — `new Error(msg)` (and the TypeError/RangeError/… family) lowers to `msg` itself: `throw`/`catch`/`String(e)` all carry the message, but `e.message`/`e.name`/`e.stack` read `undefined` (no error object exists). Runtime-raised errors (bounds checks, coercion, JSON/URI/base64/hex parsing, …) each carry a distinct small integer code (`src/err-codes.js`, ~48 sites grouped 1xx TypeError/2xx RangeError/3xx SyntaxError·URIError) instead of one shared sentinel — in-wasm `catch` still sees the raw code, and a throw that escapes to the host (no user `try`/`catch` required) decodes to a real instance of the right ECMAScript class (`e instanceof SyntaxError`, etc.) with `e.message` and `e.thrown` (the raw code) set. No-EH modules still lower internal throws to `unreachable` traps for wasm-MVP portability, kept decodable by a small mutable-i64 marker global (`__jz_last_err_bits`) that `interop.js`'s `decodeThrown` reads host-side. A genuine foreign trap (OOB, stack overflow, …) leaves the marker at 0 and surfaces as a bare `WebAssembly.RuntimeError`. Pinned in test/errors.js.
