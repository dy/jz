...
- **Errors are their message strings** — `new Error(msg)` (and the TypeError/RangeError/… family) lowers to `msg` itself: `throw`/`catch`/`String(e)` all carry the message, but `e.message`/`e.name`/`e.stack` read `undefined` (no error object exists). Runtime-raised errors (bounds checks, coercion, JSON/URI/base64/hex parsing, …) each carry a distinct small integer code (`src/err-codes.js`, ~48 sites grouped 1xx TypeError/2xx RangeError/3xx SyntaxError·URIError) instead of one shared sentinel — in-wasm `catch` still sees the raw code, but any throw that escapes to the host is resolved back to a real `Error` with the right ECMAScript class name and message (`interop.js`'s `decodeThrown`, zero wasm cost — the table lives host-side). Pinned in test/errors.js.
