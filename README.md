...
- **Errors you construct are real objects; internal ones are still codes** – `new Error(msg)` (and the TypeError/RangeError/SyntaxError/ReferenceError/URIError/EvalError family, with or without `new`) builds an object with `.message`, `.name`, and sound `instanceof` (siblings never satisfy each other; `e instanceof Error` is true for all 7). A runtime-raised error you didn't construct (bounds checks, `JSON.parse`, …) is still a raw internal code inside an in-wasm `catch`: `.message`/`.name` read `undefined` and `instanceof` is `false`, not a guess. `null.x` still yields `undefined` instead of throwing.
