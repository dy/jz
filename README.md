...
- **Generators (sync)** — `function*`/`yield` compile to regenerator-style state machines (no stack suspension): `next(v)`/`return(v)` are ordinary closure calls, `for-of` over a generator call desugars to a plain loop. v1 scope: no `yield*`, no `try` across yield — both reject with precise messages.
