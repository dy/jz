...
- **Errors you construct are real objects; internal ones are still codes** – `new Error(msg)` (and the TypeError/RangeError/SyntaxError/ReferenceError/URIError/EvalError family, with or without `new`) builds an object with `.message`, `.name`, and sound `instanceof` (siblings never satisfy each other; `e instanceof Error` is true for all 7). `.message`/`.name` are enumerable — `Object.keys`, `JSON.stringify`, spread, and `for-in` all see them; real JS makes both non-enumerable. Deliberate: objects carry values only, no per-property metadata (see below), so Error is an ordinary object on every surface rather than the one exception. A runtime-raised error you didn't construct (bounds checks, `JSON.parse`, …) is still a raw internal code inside an in-wasm `catch`: `.message`/`.name` read `undefined` and `instanceof` is `false`, not a guess — one exception: a member access or call on a receiver proven possibly nullish raises a real TypeError, `.name`/`.message`/`instanceof` all sound, same as one you construct yourself. `null.x` (a literal, not a proven-nullish binding) still yields `undefined` instead of throwing.
...
- **Internal errors stay numeric codes; your own are real objects** – runtime-raised errors (bounds checks, coercion, JSON/URI/base64/hex parsing, …) each carry a distinct small integer code (`src/err-codes.js`, ~48 sites grouped 1xx TypeError/2xx RangeError/3xx SyntaxError·URIError) instead of one shared sentinel: in-wasm `catch` sees the raw code, `e.message`/`e.name` read `undefined`, and `e instanceof AnyErrorClass` is honestly `false` (a jz-internal code and a user's own `throw <sameNumber>` are otherwise indistinguishable numbers – no numeric range can tell them apart soundly, so jz doesn't guess). The one runtime-raised family that's a real object, not a code: a member access or call on a receiver proven possibly nullish (see above). A throw that escapes to the host (no user `try`/`catch` required) still decodes to a real instance of the right ECMAScript class (`e instanceof SyntaxError`, etc.) with `e.message` and `e.thrown` (the raw code) set. Errors you construct yourself (`new Error(msg)`/`TypeError`/`RangeError`/…) are real objects throughout, in-wasm and at the host boundary — thrown *or* simply `return`ed from an export, both decode to the real host class with `.message`/`.name` intact. No-EH modules still lower internal throws to `unreachable` traps for wasm-MVP portability, kept decodable by a small mutable-i64 marker global (`__jz_last_err_bits`) that `interop.js`'s `decodeThrown` reads host-side. A genuine foreign trap (OOB, stack overflow, …) leaves the marker at 0 and surfaces as a bare `WebAssembly.RuntimeError`. (An `async` export that *resolves* — not throws — with a constructed Error is a known gap: the promise-value path doesn't carry a heap value through correctly yet, independent of this decode.) Pinned in test/errors.js.
...
One known divergence class: BigInt values are raw i64 bits in an f64 slot, and small-magnitude bit patterns collide with subnormal Numbers (`1n` and `5e-324` are the same 64 bits). `Number` coercion (`__to_num`, incl. unary `+`) used to resolve that ambiguity with an unconditional heuristic — any nonzero finite subnormal reaching an unproven-kind coercion was assumed to be a raw BigInt carrier — which misread genuine subnormal Numbers (`+Number.MIN_VALUE`, `-5e-324` literals, even inside the self-hosted `dist/jz.wasm` compiler's own internals, since its source is itself BigInt-free) as their colliding bit-pattern BigInt instead (audit-#11 P0-1). Fixed for every program that never constructs a BigInt anywhere (`ctx.features.bigint` gates the heuristic off there — the carrier can't occur if the program can't build one): every subnormal Number, literal or computed, native or self-hosted kernel, is now exact. A tagged-pointer BigInt representation (PTR.BIGINT carrier boxing, `.work/carrier-representation-design.md`) is the default as of §34 (FLIP-READY: every named gap closed, self-hosted-kernel verification fully green) — `JZ_CARRIER_BOX=0` opts back out to the legacy raw-i64 carrier, for A/B or rollback. Boxing removes the ambiguity for every slot the write-side census can reason about (schema fields, proven-kind locals/params). One coarser divergence survives the flip either way, unrelated to the flag: `__to_num`'s ambiguity check is gated on `ctx.features.bigint`, a *whole-program* fact ("can this program construct a BigInt anywhere"), not a per-slot one — so a value's kind that's genuinely unproven at the coercion site (a dict-shaped property or mixed-type array element, not a plain local/param) still risks a false-positive BigInt read even when that specific slot never holds one — pinned as an expected, permanent divergence in `test/data.js`.
