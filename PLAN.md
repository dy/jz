...
- [x] web-audio-api compiles and renders (2026-09-02): an offline oscillator through a gain renders bit-identical to Node (4410 frames, peak 0.5, RMS 0.3532364194267883) with `@audio/decode`, `@audio/speaker`, `AudioWorklet` and `pcm-convert` as host imports. The classes it took: class and literal accessors as `x__get`/`x__set` slots with a probe only where a derived class installs one; `try` across `yield`/`await`; destructuring in generator bodies; module-level `await import('x')` bundled; `external` specifiers as host imports with `undefined` padding in the i64 carrier; standard modules in jz's own subset (`src/std`: `jz:events`, `jz:domexception`, `jz:weakref`) imported implicitly by a free reference, and the promise, iterator and URLSearchParams runtimes moved onto the same mechanism so a program holds one copy and one microtask queue (the per-module splices deadlocked an await across modules); a descriptor-free reflection model (`C.prototype` is `{}`, data descriptors, `defineProperty` stores); `'x' in o` through accessor slots; `Func || Fallback` folds in prepare; `Symbol.dispose ||= …` is a no-op. What it costs today: 814 KB of wasm from 246 KB of source and a 9 s compile; a 5 s stereo saw → biquad → gain render takes 65 ms in jz against 6 ms in V8. The profile is the `any` tier, not the DSP: `__len`, `__typed_idx`, `__typed_set_idx`, `__typed_shift` and `__dyn_get_t_h` take 60% of the samples, because every `out[i]` in a node's `_process` indexes a channel that arrived from `buf.getChannelData(c)` with no static kind (the largest method, 690 KB of WAT, is ConvolverNode's). The flagship's typed tier (phase 4) is where its size and speed come from: a method's return kind on a fixed-shape class (`getChannelData` returns `Float32Array`) so the loops index typed storage directly, or, until then, a loop versioned once on the runtime kind of its invariant receivers (`unswitchTypedParamLoop` does this for Float64Array params only)
...
- [ ] web-audio-api runs as the flagship (it compiles and renders, see v1): a long-lived node graph in the session region, DSP kernels typed with nothing escaping, the node classes on fixed shapes instead of `any`-tier closures (814 KB → the library's own size class), `pcm-convert`'s constructors-as-values (`DTYPE[x].C`) typed, the report showing the boundary and the regions
