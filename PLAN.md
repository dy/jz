...
| --- | --- |
| resample | General gather-map SIMD now packs the scalar checked reads and lifts their pure Float64 arithmetic suffix. Each floating phase addition stays in scalar order; distinct, unwritten owned parameters prove independence and the original loop handles odd tails. Aliases, views, live-out suffix values and cheap suffixes decline. Speed binary 2336 → 3492 bytes, checksum unchanged. The first ten-round CI probe ([36185720511](https://github.com/dy/jz/actions/runs/36185720511), EPYC 7763, Node 24) leads JSC in all ten pairs: median JZ/JSC 0.9732, range 0.9118–0.9838; Bun 0.9713, V8 0.9088, C-Wasm 0.7321, Rust-Wasm 0.2351. This is a narrow margin, not yet a moat. AssemblyScript was missing from the probe PATH; corrected for the repeat. |
| sdf | Sentinel copies already preserve summary kinds. Load-CSE now initializes numeric scratch with an unobservable zero, allowing proven-present integer loads to retain integer storage while checked misses remain undefined. Interval analysis retains an assignment expression's result, preserving bounds through the first cached gather. Speed binary 3427 → 3185 bytes; all 62 corpus checksums and the fresh self-compile gate pass. The same ten-round CI probe leads JSC in every pair: median 0.9493, range 0.9376–0.9555; Bun 0.9319, V8 0.6830, C-Wasm 0.8952, Rust-Wasm 0.9652. Repeat with AssemblyScript coverage and keep this row open until the lead has a wider margin. |
