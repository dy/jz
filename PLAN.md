...
| Correctness | Registry self-compile passes 76 tests / 2496 assertions. WAT token storage passes 125 assertions across tiers; printer allocation fell from 2850519192 to 128312744 bytes. CI at bd3147fb passes all four matrix legs, fuzz and both conformance jobs. Its Wasm-hosted suite passed 3823 / 3824 tests; the remaining allocation failure came from the test adapter dropping `_compactCollections`. Forwarding it passes kernel allocation 15 / 570 and two formerly skipped binary/WAT layout tests, 2 / 45. Harness regressions pass 7 / 66, Porffor recovery 1 / 54 and merge 23 / 186. | Finish the full core and Wasm-hosted reruns and require green self-compile CI after the adapter repair. |
