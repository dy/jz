...
2. **The tape.** The IR tape enters the pipeline between emit and optimize: every function is decoded into it and encoded back, byte-identical. Then each optimizer pass is ported to the tape and its WAT-array version deleted, smallest first, until `src/optimize` holds only tape passes and watr's optimize pass is off. Then emit builds the tape directly and the WAT-array helpers (`src/ir/*`) go. Deletes: `src/optimize` matchers, `src/ir` array helpers, the expando facts (`.type`, `.ptrKind`, `.valKind`, `.unsigned`, `.typedLen`, `.schemaSid`, `.saArr`, `.range`), watr optimize.
