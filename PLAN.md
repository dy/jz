...
| --- | --- |
| resample | General gather-map SIMD now packs the scalar checked reads and lifts their pure Float64 arithmetic suffix. Each floating phase addition stays in scalar order; distinct, unwritten owned parameters prove independence and the original loop handles odd tails. Aliases, views, live-out suffix values and cheap suffixes decline. Speed binary 2336 → 3492 bytes, checksum unchanged. Local paired timings are too noisy for a leadership claim. The manual bench-probe workflow now includes Bun/JSC and C/Rust/AS Wasm rivals; require repeatable CI gains before closing this row. |
| sdf | Sentinel copies already preserve summary kinds. Load-CSE now initializes numeric scratch with an unobservable zero, allowing proven-present integer loads to retain integer storage while checked misses remain undefined. Interval analysis retains an assignment expression's result, preserving bounds through the first cached gather. Speed binary 3427 → 3185 bytes, checksum validation and full gates in progress. Require repeated paired evidence against JSC and rival Wasm before closing this row. |
