...
export let frame = (t) => {
  // rows to compute this frame: ceil(H/45)
  let batchSize = ((H + 44) / 45) | 0
  ...
  // scratch Float64Array for RK4 per-pixel state — single pixel at a time
  // allocate in frame is fine since it's per row-batch not per pixel per frame

  while (row < endRow) {
    let col = 0
    while (col < W) {
      // map (col, row) → (θ1, θ2) in [-π, π]
      let th1 = ((col / W) * 2.0 - 1.0) * Math.PI
      let th2 = ((row / H) * 2.0 - 1.0) * Math.PI
      let w1 = 0.0, w2 = 0.0

      // track unwrapped θ2 to detect flip
      let th2_unwrap = th2
      let flipStep = 0  // 0 = no flip yet
      ...
      while (step < MAX_STEPS) {
        ...
        // unwrap θ2: track cumulative rotation
        ...
        // wrap th1 and th2 to [-π,π] for numerics
      ...
      // color pixel
      if (flipStep > 0) {
        px[row * W + col] = flipColor(flipStep)
      } else {
        px[row * W + col] = (255 << 24)  // black = never flipped
      }
      col++
    }
    row++
