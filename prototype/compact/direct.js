...
// Frozen direct-binary control for the compact experiment. Production work uses
// the staged WAT path in compiler.js. Keep this file feature-frozen so it remains
// an independent size and semantics lower bound.
const F_NAME = 0
const F_PARAMS = 1
const F_BODY = 2
const F_EXPORT = 3
...
export function compileDirectAst(ast) {
  const top = list(ast)
  const funcs = []
...
export default function compileDirect(source) {
  return compileDirectAst(parse(source))
}
