...
Latest loaded-machine result against the fresh 14,483,762-byte `dist/jz.wasm`:

- staged compiler: 2,256,496 bytes, 6.42x smaller
- staged source graph: 72 modules, 1,016,513 source bytes
- compile-speed geomean: 36.95x
- minimum compile speedup: 4.16x
...
- the typed SIMD row compiles 12.63x faster and emits 287 bytes versus production's 568 bytes
...
The machine had about 12.1 GiB of allocated swap, so these timings do not certify release performance. The artifact ratio compares compilers with different language coverage and does not predict production savings.
