...
Latest loaded-machine result against the fresh 14,501,939-byte `dist/jz.wasm`:

- staged compiler: 2,256,528 bytes, 6.43x smaller
- staged source graph: 72 modules, 1,016,861 source bytes
- compile-speed geomean: 40.01x
- minimum compile speedup: 4.64x
...
- the typed SIMD row compiles 15.78x faster and emits 287 bytes versus production's 568 bytes
...
The machine had about 11.9 GiB of allocated swap, so these timings do not certify release performance. The artifact ratio compares compilers with different language coverage and does not predict production savings.
